...
def test_init():
    eq_(('#pyfibot', 'feed added with 50 items'), command_rss(bot, ADMIN_USER, '#pyfibot', 'add ./pyfibot/tests/static/test_rss_init.xml'))
    eq_(
...
def test_update():
    global f
    f = Feed('nerv', '#pyfibot', url=r'./pyfibot/tests/static/test_rss_init.xml')
    f.update_feed_info({'url': r'./pyfibot/tests/static/test_rss_check.xml'})
    eq_(
        None,
...
def test_command_remove_4():
    eq_(
        ('#pyfibot', 'feed "Uutiset - Ampparit.com" <./pyfibot/tests/static/test_rss_check.xml> removed'),
...
def test_command_add():
    check_re(
        r'feed added with \d+ items',
        command_rss(bot, ADMIN_USER, '#pyfibot', 'add ./pyfibot/tests/static/test_rss_init.xml')[1]
    )
...
def test_command_add_2():
    eq_(
        ...
        command_rss(bot, ADMIN_USER, '#pyfibot', 'add ./pyfibot/tests/static/test_rss_init.xml')
    )
