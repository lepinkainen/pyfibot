...
def handle_args(args, channel):
    ...
    )  # regex for validating nick format
        ...
        if len(args) > 0:
            ...
            if cmp(op_channel, user) != 0:
                for nick in nicks:
                    if (
                        nick in oplist
                    ):  # if already in oplist, just add to existing list
                        if (
                            op_channel in oplist[nick]
                        ):  # if not in nicks channel list, add
                            message += "Auto-op for %s already in %s. " % (
                                nick,
                                op_channel,
                            )
                        else:
            ...
            if cmp(op_channel, user) != 0:
                for nick in nicks:
                    if nick in oplist:
                        if (
                            op_channel in oplist[nick]
                        ):  # if in nicks channel list, remove
                            message += "Auto-op for %s in %s removed. " % (
                                nick,
                                op_channel,
                            )
...
def autoop_list(user, channel, args):
    if isAdmin(user):
        ...
        if (
            len(args) == 0
            or (re.match(rg, args[0].strip()) is None and args[0].strip()[0] == "#")
        ) and args[0] != "me":
            if len(args) == 0:
                op_channel = channel
