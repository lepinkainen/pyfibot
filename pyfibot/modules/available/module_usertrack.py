...
def handle_userJoined(bot, user, channel):
    ...
    table = get_table(bot, channel)
    if table.find_one(
        nick=getNick(user), ident=getIdent(user), host=getHost(user), op=True
    ):
...
def handle_userLeft(bot, user, channel, message):
    ...
    # QUIT returns the channel as None, loop through all tables, check if user exists and update it
        ...
        for t in db.tables:
            ...
            res = table.find_one(
                nick=getNick(user), ident=getIdent(user), host=getHost(user)
            )
            if res:
                data["id"] = res["id"]
...
def handle_userRenamed(bot, user, newnick):
    ...
    for t in db.tables:
        ...
        if table.find_one(nick=nick, ident=ident, host=host) or table.find_one(
            nick=newnick, ident=ident, host=host
        ):
...
def command_add_op(bot, user, channel, args):
    ...
    return bot.say(
        channel, "auto-opping %s!%s@%s" % (res["nick"], res["ident"], res["host"])
    )
...
def command_remove_op(bot, user, channel, args):
    ...
    return bot.say(
        channel,
        "removed auto-op from %s!%s@%s" % (res["nick"], res["ident"], res["host"]),
    )
...
def command_list_ops(bot, user, channel, args):
    ...
    if args.strip() == "full":
        ops = ", ".join(
            [
                "%s!%s@%s" % (r["nick"], r["ident"], r["host"])
                for r in table.find(op=True)
            ]
        )
    else:
...
def command_seen(bot, user, channel, args):
    ...
    if user["last_action"] in ["left", "quit"]:
        return bot.say(
            channel,
            "%s was last seen at %s (%s ago) [%s, %s]"
            % (
                user["nick"],
