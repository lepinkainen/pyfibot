...
class NuggetsEn(Nuggets):
	def __init__(self, archive_num):
		...
		self.url = "http://en.wikipedia.org/wiki/Wikipedia:Recent_additions_" + archive_num
...
class NuggetsEn(Nuggets):
	def __init__(self, archive_num):
		...
		self.archive_max = 200
		self.archive_num = archive_num
		if self.archive_num > self.archive_max: self.archive_num = random.randint(1, self.archive_max)
		self.url = "http://en.wikipedia.org/wiki/Wikipedia:Recent_additions_200"
		Nuggets.__init__(self)
	def getNuggets(self):
		self.makeSoup()
		ugly_nugget_list= []
